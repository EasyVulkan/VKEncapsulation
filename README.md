...
如果你有使用一些其他的Vulkan官方或第三方库，光看这个列表应该就能明白这个库的核心部分 [Core (文档施工中)]() 的功能定位：
...
* 利用 volk 获取所有函数的指针，无需手动获取扩展功能的函数指针（且 [Core/VKOop.h (文档施工中)]() 中的类成员函数会根据Vulkan运行时版本选择调用有/无扩展后缀的函数）。
* 封装了初始化流程，实现简单且可定制的初始化。
* 为 *vulkan_core.h* 提供C++绑定，简化参数填写、简化函数调用。

在此基础上有：

* [Core (文档施工中)]() 中的各个文件对 *vulkan_core.h* 提供不同程度的面向对象封装，你可以根据自己的编程风格选择使用部分功能和特性。
* [Plus (文档施工中)]() 中的文件提供功能上更加特定的封装。
* [WindowSystem (文档施工中)]() 中的文件提供了执行整个初始化流程的函数封装，以及其它与窗口系统有关的常用功能的函数封装（目前只有对应 [Glfw](https://www.glfw.org/) 和 Win32 API 的版本，因为我只有Win10系统的主机）。<br>
* 近似 C# 的命名风格。
...
CmdPushDescriptorSet(VK_PIPELINE_BIND_POINT_GRAPHICS, pipelineLayout_composition, 0, { // 省略了参数中的 commandBuffer ，自动填写为当前线程中正在录制的命令缓冲区
    WriteDescriptorSet{}.
...
* 会在线程中切换逻辑设备的程序
    ...
    允许在一个线程中重建逻辑设备，但不能来回切换不同逻辑设备，这是为了在满足 “[基于线程的状态机 (文档施工中)]()” 的同时确保安全性并简化程序设计。
...
* 需要多个逻辑设备的程序
    ...
    这通常意味着你想使用多个物理设备。<br>
    由于这会将代码复杂化，且个人开发者不太会写这类程序，因此 [Core/VKAppBase.h (文档施工中)]() 中涉及逻辑设备的函数一概只支持单个逻辑设备的情形。<br>
...
* 需要多个交换链的程序
    ...
    * 右键菜单也可以完全显示在主窗口内（VS Code 正是如此）。
    ...
    出现以上这些情况的一个可能的原因，或许是因为创建子窗口需要使用特定操作系统提供的API，而使用Vulkan这种跨平台图形API则更多是为了考虑可移植性。

    如果你需要多个交换链，你可以自行将 [Core/VKAppBase.h (文档施工中)]() 中与交换链相关的的变量和函数封装进一个类（可能会有点麻烦哦！），然后再创建该类实例的数组。
...
本项目不使用版本号，仅在这个文档中标定 **[Core (文档施工中)]() 和 [Plus (文档施工中)]() 两个文件夹**中任意文件的最后更新日期（可看成是以日期为版本号）。

### 更新频率

目前本项目由我个人维护，增量更新的频率注定会很低。<br>
（我不介意你贡献代码！细则参见 [CONTRIBUTING (文档施工中)](/CONTRIBUTING.md) ）

[Core (文档施工中)]() 文件夹中代码的下一次增量更新可能会是 Vulkan SDK 更新到 **1.5.XXX** 的时候。<br>
...
* [VKEncapsulation 编程指南 (文档施工中)]()

    因为还没写，关于依赖项的要求先写在这里

    如果你不想[自定义依赖项路径 (todo)]()，Dependency文件夹的子目录必须为：

    * **Vulkan** // 以下文件夹你全部可以从Vulkan SDK的安装目录找到，GLM和volk需要在安装时手动选择
        * **Include** // 设置为附加包含目录
            * vulkan
            * vk_video
            * shaderc
            * glm
            * Volk
    * **GLFW** // 从[GLFW官网](https://www.glfw.org/download.html)下载预编译binaries后，以下文件夹应该在压缩包根目录中
        * **include** // 设置为附加包含目录
            * GLFW
        * **lib_win64** // 设置为附加库目录
    * **stb_image.h**（单个文件，[下载地址](https://github.com/nothings/stb/blob/master/stb_image.h)）

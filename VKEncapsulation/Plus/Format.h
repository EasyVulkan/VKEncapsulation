...
struct FormatInfo {
	static constexpr uint8_t formatInfos_v1_0[][4] = {
	...
	uint8_t rawDataType;
	FormatInfo() = default;
	constexpr FormatInfo(Format format) {
	#ifndef NDEBUG
		if (!Between_Open<int32_t>(0, format, std::size(formatInfos_v1_0)))
			OutputMessage("[ vke::ext::FormatInfo ] ERROR\nThis function only supports definite formats provided by VK_API_VERSION_1_0.\n"),
			abort();
	#endif
		componentCount = formatInfos_v1_0[format][0];
		sizePerComponent = formatInfos_v1_0[format][1];
		sizePerPixel = formatInfos_v1_0[format][2];
		rawDataType = formatInfos_v1_0[format][3];
	}
};

constexpr Format Corresponding16BitFloatFormat(Format format_32BitFloat) {

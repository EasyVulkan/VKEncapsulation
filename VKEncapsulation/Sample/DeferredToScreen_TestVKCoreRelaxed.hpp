...
#define VK_ENCAPSULATION_ALLOW_RAII_FUNCTIONS
...
struct DeferredToScreen {
	...
	static void Initialize() {
		...
		descriptorSetLayout_gBuffer.Create().
			...
			Bindings(
				DescriptorSetLayoutBinding{ 0, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, 1, VK_SHADER_STAGE_VERTEX_BIT });
		pipelineLayout_gBuffer.Create().
			SetLayouts(descriptorSetLayout_gBuffer);
		/* Composition */
		descriptorSetLayout_composition.Create().
			Flags(VK_DESCRIPTOR_SET_LAYOUT_CREATE_PUSH_DESCRIPTOR_BIT).
			Bindings({
		...
		pipelineLayout_composition.Create().
	...
	static void CmdBeginRendering() {
		...
		CmdSetRenderingAttachmentLocations().ColorAttachmentLocations({ VK_ATTACHMENT_UNUSED, 0u, 1u });
	}
	static void CmdSetRenderingInputAttachmentIndices() {
		...
		vke::CmdSetRenderingInputAttachmentIndices().ColorAttachmentInputIndices({ VK_ATTACHMENT_UNUSED, 0u, 1u });
		CmdSetRenderingAttachmentLocations().ColorAttachmentLocations({ 0u, VK_ATTACHMENT_UNUSED, VK_ATTACHMENT_UNUSED });
	}
	...
	while (!WindowShouldClose()) {
		...
		CmdPushDescriptorSet(VK_PIPELINE_BIND_POINT_GRAPHICS, DeferredToScreen::pipelineLayout_composition, 0, {
			WriteDescriptorSet{}.
				DescriptorType(VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER).
				BufferInfo(
					DescriptorBufferInfo{}.Buffer(uniformBuffer).Range(VK_WHOLE_SIZE)),
			WriteDescriptorSet{}.
				DstBinding(1).
				DescriptorType(VK_DESCRIPTOR_TYPE_INPUT_ATTACHMENT).
				ImageInfo({
					DescriptorImageInfo{}.ImageView(DeferredToScreen::attachment_normalZ).ImageLayout(DeferredToScreen::inputAttachmentLayout),
					DescriptorImageInfo{}.ImageView(DeferredToScreen::attachment_albedoSpecular).ImageLayout(DeferredToScreen::inputAttachmentLayout) }) });
